...
public class JuegoBattleship {
    ...
    public JuegoBattleship(Socket socketMock) {
    ...
    private boolean puedeColocarBarco(int fila, int columna, int tamanio, boolean horizontal) {
        if (horizontal) {
            if (columna + tamanio > TAMANIO_TABLERO)
                return false;
            for (int i = columna; i < columna + tamanio; i++) {
                if (tableroPropio[fila][i] != '~')
                    return false;
            }
        } else {
            if (fila + tamanio > TAMANIO_TABLERO)
                return false;
            for (int i = fila; i < fila + tamanio; i++) {
                if (tableroPropio[i][columna] != '~')
                    return false;
            }
        }
        return true;
    ...
    public boolean recibirDisparo(int fila, int columna) {
        ...
        boolean impacto = tableroPropio[fila][columna] != '~';
